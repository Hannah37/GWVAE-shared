...
class Filter(object):
    ...
    * Initialize attributes for derived classes.
    ...
    def evaluate(self, x):
        ...
        y = torch.empty([self.Nf] + list(x.shape))
    ...
    def filter(self, s, method='chebyshev', order=15):
        ...
        if method == 'exact':
            ...
            if n_features_in == 1:  # Analysis.
                ...
                s = reshape_fortran(s, (self.G.N, n_features_out, n_signals))
                s = s.swapaxes(1, 2)
    ...
    def localize(self, i, **kwargs):
        ...
        """
        # s = np.zeros(self.G.N)
        s = torch.zeros(self.G.N).cuda()
        s[i] = 1
        # return np.sqrt(self.G.N) * self.filter(s, **kwargs)
        return torch.sqrt(self.G.N) * self.filter(s, **kwargs)
    ...
    def estimate_frame_bounds(self, x=None):
        ...
        """
        if x is None:
            x = torch.linspace(0, self.G.lmax, 1000).cuda()
        else:
            x = np.asanyarray(x)
        ...
        sum_filters = torch.sum(self.evaluate(x)**2, axis=0)
    ...
    def compute_frame(self, **kwargs): ##
        ...
        *analysis operator*
        ...
        s = torch.eye(self.G.N).cuda()
        # return self.filter(s, **kwargs).T.reshape(-1, self.G.N)
        out = self.filter(s, **kwargs).T.reshape(-1, self.G.N)
        return out
    ...
    def inverse(self):
        ...
        *synthesis filter bank* :math:`g^+` such that
        ...
        def kernel(g, i, x):
            ...
            z = torch.squeeze(torch.linalg.pinv(torch.unsqueeze(y, axis=-1)), axis=-2).cuda()
