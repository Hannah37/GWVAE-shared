...
def compute_cheby_coeff(f, m=30, N=None, *args, **kwargs):
    ...
    # c = np.zeros(m + 1)
    c = torch.zeros(m + 1).cuda()

    tmpN = torch.arange(N).cuda()
    num = torch.cos(torch.pi * (tmpN + 0.5) / N).cuda()
    for o in range(m + 1):
        ...
        # tmpN = torch.tensor(tmpN).cuda()

        # c[o] = 2. / N * torch.dot(torch.tensor((f._kernels[i](a1 * num + a2))).float().cuda(),
        #                        torch.tensor((torch.cos(torch.pi * o * (tmpN + 0.5) / N))).float().cuda())

        c[o] = 2. / N * torch.dot((f._kernels[i](a1 * num + a2)).float().cuda(),
                               (torch.cos(torch.pi * o * (tmpN + 0.5) / N)).float().cuda())
    return c
...
def cheby_op(G, c, signal, **kwargs):
    ...
    Nv = signal.shape[1]
    r = torch.zeros((G.N * Nscales, Nv)).cuda().float()
    ...
    # r = torch.tensor(r).cuda().float()

    a_arange = [0, G.lmax]

    a1 = torch.tensor(float(a_arange[1] - a_arange[0]) / 2.).cuda()
    a2 = torch.tensor(float(a_arange[1] + a_arange[0]) / 2.).cuda()

    twf_old = signal
    # print(G.L)
    # print(G.L.dot(signal))
    gl_coo = G.L.tocoo()
    gl_torch = torch.sparse.LongTensor(torch.LongTensor([gl_coo.row.tolist(), gl_coo.col.tolist()]),
                              torch.LongTensor(gl_coo.data)).cuda().float()
    
    
    twf_cur = (torch.sparse.mm(gl_torch, signal) - a2 * signal) / a1

    tmpN = torch.arange(G.N, dtype=int)
    for i in range(Nscales):
        r[tmpN + G.N*i] = 0.5 * c[i, 0] * twf_old + c[i, 1] * twf_cur

    # factor = 2/a1 * (G.L - a2 * sparse.eye(G.N))
    factor =  -2/a1 * (a2 * torch.eye(G.N).cuda() - gl_torch)
    for k in range(2, M):
        # twf_new = factor.dot(twf_cur) - twf_old
        twf_new = torch.mm(factor, twf_cur) - twf_old
        for i in range(Nscales):
