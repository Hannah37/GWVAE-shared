...
class Heat(Filter):
    ...
    def __init__(self, G, tau=10, normalize=False, **kwargs):
        ...
        def kernel(x, t):
            ...
            # return torch.exp(torch.tensor(-t * x / G.lmax)).cuda()
            return torch.exp(-t * x / G.lmax)

        g = []
        for t in tau:
            # norm = np.linalg.norm(kernel(G.e, t)) if normalize else 1
            # print('t: ', t.requires_grad) # True
            norm = torch.linalg.norm(kernel(G.e, t)) if normalize else 1
